...
export async function GET() {
  ...
  const { data, error } = await admin
    .from("users")
    .select("id, phone_e164, tier, access_status, trial_ends_at, created_at, last_active_at, source, stripe_customer_id")
    .is("deleted_at", null)
  ...
  const header = [
    "id","phone_masked","tier","access_status","trial_ends_at",
    "created_at","last_active_at","source","stripe_customer_id",
  ];
