...
async function generateReply({ user, body, isFirstInteraction, correlationId = null, messageId = null }) {
  ...
  await record({ correlationId, stage: STAGES.CONVERSATION_LOADED, status: STATUS.SUCCESS, userId: user.id, messageId, conversationId });

  // Bible RAG: find relevant verses for the user's message
  let bibleContext = null;
  try {
    const ragStage = await startStage({ correlationId, stage: STAGES.BIBLE_RAG_STARTED, provider: 'openai', userId: user.id, messageId });
    ...
    await record({ correlationId, stage: STAGES.BIBLE_RAG_FAILED, status: STATUS.FAILED, userId: user.id, messageId, errorMessage: err.message });
  }
  ...
  let result;
  const aiStage = await startStage({ correlationId, stage: STAGES.AI_REQUEST_STARTED, provider: process.env.AI_PROVIDER || 'deepseek', userId: user.id, messageId });
  try {
    result = await ai.generate({ messages, system: systemPrompt });
  } catch (err) {
    await completeStage(aiStage, { status: STATUS.FAILED, errorCode: err.code || 'ai_error', errorMessage: err.message });
    await record({ correlationId, stage: STAGES.AI_REQUEST_FAILED, status: STATUS.FAILED, userId: user.id, messageId, provider: process.env.AI_PROVIDER || 'deepseek', errorCode: err.code || 'ai_error', errorMessage: err.message });
    throw err;
  }
  ...
  // Send SMS
  let smsResult;
  const smsStage = await startStage({ correlationId, stage: STAGES.SMS_SEND_STARTED, provider: process.env.SMS_PROVIDER || 'smsgate', userId: user.id, messageId });
  try {
    const sms = getSmsProvider();
    smsResult = await sms.send({ to: user.phone_e164, text });
  } catch (err) {
    await completeStage(smsStage, { status: STATUS.FAILED, errorCode: err.code || 'sms_error', errorMessage: err.message });
    await record({ correlationId, stage: STAGES.SMS_SEND_FAILED, status: STATUS.FAILED, userId: user.id, messageId, provider: process.env.SMS_PROVIDER || 'smsgate', errorCode: err.code || 'sms_error', errorMessage: err.message });
    throw err;
  }
  const segments = estimateSegments(text);
  const smsCost = await computeSmsCostCents({ segments, direction: 'outbound' });

  const { data: outboundSms } = await supabase.from('sms_messages').insert({
    user_id: user.id,
    conversation_id: conversationId,
    direction: 'outbound',
    from_e164: null,
  ...
  await completeStage(smsStage, { status: STATUS.SUCCESS, metadata: { providerMessageId: smsResult.providerMessageId, messageId: outboundSms?.id ?? null } });
  ...
  return {
    ...
    outboundMessageId: outboundSms?.id ?? null,
    tokens: {
