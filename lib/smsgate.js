...
//
// Local mode: the Android app exposes an HTTP API on the device IP.
// We use Basic Auth (legacy) because it matches the user's current setup.

const http = require('http');

function getConfig() {
  return {
    url: (process.env.SMSGATE_URL || 'http://192.168.15.2:8080').replace(/\/$/, ''),
...
async function sendSms({ to, text }) {
  const { body } = await requestJson('/message', {
    method: 'POST',
    body: {
      textMessage: { text },
      phoneNumbers: [to],
...
/**
 * Register a webhook on the SMSGate device.
 * @param {Object} opts
...
async function registerWebhook({ url, event, deviceId }) {
  ...
  return requestJson('/webhooks', { method: 'POST', body: payload });
}
...
async function listWebhooks() {
  return requestJson('/webhooks', { method: 'GET' });
}
